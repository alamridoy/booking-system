...
let getRoomSearching = (sercheingFieldObject = {}) => {
    ...
    // Generate Query for Dicussion type & Vehical type (task_vehicle_types)
    ...
    if (extraWhere.length > 0) {
        if (where.length < 1) where = " where task.organization_id = ?  and ";
        else where += " and task.organization_id = ?  and ";
    }

    // add extra_where  (extra_where = call date fielter)
