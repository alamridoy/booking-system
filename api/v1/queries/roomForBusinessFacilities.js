...
let table_name = "mtl_room_for_business_facilities_types";
...
let getByRoomType = () => {
    return `SELECT * FROM ${table_name} where  room_type = ? and status != 0`;
}
...
let getDataByBusinessTypeId = () => {
    return `SELECT business_facilities_types_id FROM ${table_name} where  room_id = ? and status != 0`;
}
...
// let getByRoomId = () => {
//     return `SELECT room_id FROM ${table_name} where  room_id = ? and status != 0`;
// }
...
let addNewRoomForBusinessData = () => {
    return `INSERT INTO ${table_name} SET ?`;
}

let addNewRoomForBusiness = () => {
    return `INSERT INTO ${table_name} SET ?`;
}
...
let updateDataForBusinessFacilitiesId = (data) => {
    let keys = Object.keys(data);
...
let getDataByFilter = (expertTypeId = []) => {

    let query = `SELECT * FROM ${table_name}  where status = 1 `;

    if (expertTypeId.length > 0) {
        query += ` and FIND_IN_SET('${expertTypeId[0]}', expert_type_id) `        
        for (let i = 1; i < expertTypeId.length; i++) {    
            query += ` or FIND_IN_SET('${expertTypeId[i]}', expert_type_id)`;  
        }
    }
