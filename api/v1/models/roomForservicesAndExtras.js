...
const queries = require('../queries/roomForservicesAndExtras')
...
let addNewRoomserviceAndExtras = async (info = [], conn) => {
    let connection = connectionHotelPool;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.addNewRoomserviceAndExtras(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
