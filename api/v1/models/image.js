...
const queries = require('../queries/image');
...
let getByBedType = async (bed_type = "") => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queries.getByBedType(), [bed_type], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getByName = async (name={}) => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queries.getByName(), [name], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
