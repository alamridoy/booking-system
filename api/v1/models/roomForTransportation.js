...
const queries = require('../queries//roomForTransportation')
...
let addNewRoomTransportation = async (info = [], conn) => {
    let connection = connectionHotelPool;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.addNewRoomTransportation(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
