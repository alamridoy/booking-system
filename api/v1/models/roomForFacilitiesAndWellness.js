...
const queries = require('../queries/roomForFacilitiesAndWellness')
...
let addNewRoomForFitnessWellness = async (info = [], conn) => {
    let connection = connectionHotelPool;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.addNewRoomForFitnessWellness(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
