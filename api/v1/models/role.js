...
let getRoleById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queriesRole.getRoleById(), [id], (error, result, fields) => {
...
let getRoleByIdentityId = async (identity_id = 0) => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queriesRole.getRoleByIdentityId(), [identity_id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getDataByWhereCondition = async (where = {}, orderBy = {}, limit = 2000, offset = 0, columnList = []) => {
    ...
    return new Promise((resolve, reject) => {
        connection.query(queriesRole.getDataByWhereCondition(where, orderBy, limit, offset, columnList), [...dataParameter], (error, result, fields) => {
            if (error) reject(error);
            else resolve(result);
        });
    });
...
let getDetailsByIdAndWhereIn = async (workShopId = []) => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queriesRole.getDetailsByIdAndWhereIn(), [workShopId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
