...
const queries = require('../queries/roomForGeneral')
...
let addNewRoomForGeneral = async (info = [], conn) => {
    let connection = connectionHotelPool;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.addNewRoomForGeneral(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
