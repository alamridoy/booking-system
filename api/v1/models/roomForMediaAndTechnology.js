...
const queries = require('../queries/roomForMediaAndTechnology')
...
let addNewRoomFormediaAndTechnology = async (info = [], conn) => {
    let connection = connectionHotelPool;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.addNewRoomFormediaAndTechnology(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
