...
const queries = require('../queries/roomForFoodAndDrink')
...
let addNewRoomForFoodAndDrink = async (info = [], conn) => {
    let connection = connectionHotelPool;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.addNewRoomForFoodAndDrink(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
