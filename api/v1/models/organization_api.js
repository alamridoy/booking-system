...
const queries = require('../queries/organization_api');

// Promises Method

let getList = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queries.getList(), [id], (error, result, fields) => {
...
let getByOrganizationName = async (name = "") => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queries.getByOrganizationName(), [name], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getDetailsByIdAndWhereIn = async (expertTypeId = []) => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queries.getDetailsByIdAndWhereIn(), [expertTypeId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
