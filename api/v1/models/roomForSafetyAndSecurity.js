...
const queries = require('../queries/roomForSafetyAndSecurity')
...
let addNewRoomSafetyAndSecurity = async (info = [], conn) => {
    let connection = connectionHotelPool;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.addNewRoomSafetyAndSecurity(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
