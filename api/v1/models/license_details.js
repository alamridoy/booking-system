...
const queries = require('../queries/license_details');
...
let getLicenseDetailsById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionHotelPool.query(queries.getLicenseDetailsById(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
