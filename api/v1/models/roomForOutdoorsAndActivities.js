...
const queries = require('../queries/roomForOutdoorsAndActivities')
...
let addNewRoomForoutdoorsAndActivities = async (info = [], conn) => {
    let connection = connectionHotelPool;
    if (conn !== undefined) connection = conn;

    return new Promise((resolve, reject) => {
        connection.query(queries.addNewRoomForoutdoorsAndActivities(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
