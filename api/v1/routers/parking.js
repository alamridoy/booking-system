...
router.post('/add-parking',async(req,res)=>{
    ...
    let existingData = await parkingModel.getByTitle(reqData.title);
    ...
    if (!isEmpty(existingData)) {
        return res.status(409).send({
            "success": false,
            "status": 409,
            "message": existingData[0].status == "1" ? "This Parking Already Exists." : "This Parking Already Exists but Deactivate, You can activate it."
        });
